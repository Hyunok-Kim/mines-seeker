...
Rectangle {
    ...
    property color borderBlockColor: "#10000000"
    ...
    Rectangle {
        ...
        Image {
            ...
            source: "images/Board/" +
                ((flagged && !activated) ? "flag" : (flaggedUnknown && !activated) ? "unknown" :
                (content === Cell.MINE && activated) ? "mine" : (content === Cell.VALUE && activated) ? "number" + value : "blank") + ".svg"
            ...
            Image {
                ...
                sourceSize: Qt.size(size, size)
                source: "images/Board/" +
                    ((flagged && content !== Cell.MINE && activated) ? "cross" : (flagged && content === Cell.MINE && activated) ? "check" :
                    "blank") + ".svg"
        ...
        MouseArea {
            ...
            onPressed: (mouse) => {
                if (activated && mouse.button === Qt.LeftButton && settings.oneClickRevealSiblings) {
                    if (clickRevealSiblingsRipple.pressed) {
                        clockRevealSiblingsRipple.pressed = false
                    }
            ...
            Timer {
                ...
                interval: 25
                onTriggered: {
                    if (mouseArea.preesedButtons === (Qt.RightButton | Qt.LeftButton)) {
                        clickRevealSiblingsRipple.pressed = true
                        cell.leftRightClick()
                        cell.bothClick = true
