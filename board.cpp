...
{
    if (!running && firstClick) { // diable exept when not click done yet
#ifdef DEBUG_OUTPUT
...
#ifdef DEBUG_OUTPUT
        qDebug("Cell at (%d, %d) alreay activated, left-click ignored", row, column);
#endif
    ...
    if (row < 0 || row > numberOfRows) {
        qCritical("Critical error: The specified row in ([%d], %d) is out of range (maximu allowed is %d).", row, column, numberOfRows);
    } else if (column < 0 || column > numberOfColumns) {
        qCritical("Critical error: The specified column in (%d, [%d]) is out of range (maximum is allowd is %d).", row, column, numberOfColumns);
    } else if (cells[row][column].getActivated()) {
        ...
        if (cells[row][column].getContent() == Cell::MINE) {
            lose(row, column);
...
#ifdef DEBUG_OUTPUT
        qDebug("Not running, right-click ignored");
#endif
        return; // give up
    } else if (cells[row][column].getActivated()) {
#ifdef DEBUG_OUTPUT
...
int Board::unlockAdjacentCells(int row, int column)
{
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        bool isValid = (newRow >= 0 && newRow < numberOfRows) && (newColumn >= 0 && newColumn < numberOfRows);
        if (isValid && cells[row][column].getContent() == Cell::BLANK && cells[newRow][newColumn].getContent() != Cell::MINE
            && !cells[newRow][newColumn].getActivated() && !cells[newRow][newColumn].getFlagged() &&
            !cells[newRow][newColumn].getFlaggedUnknown()) {
...
{
    triggerSiblings(row, column, true);
}

void Board::unrevealSiblings(int row, int column)
{
    triggerSiblings(row, column, false);
}

void Board::triggerSiblings(int row, int column, bool reveal)
{
