...
#define SHA_512(X) QCrytographicHash::hash((X), QCrytographicHash::Sha3_512).toHex()

RecordsManager::RecordsManager(const QString &filePath, QObject *parent)
    : QThread{parent}
{
    setupRecordTemplate();
...
{
    recordTemplate.append(QSqlField("NAME", QMetaType::fromType<QString>()));
    recordTemplate.append(QSqlField("TIME", QMetaType::fromType<int>()));
    recordTemplate.append(QSqlField("SCORE", QMetaType::fromType<int>()));
}
